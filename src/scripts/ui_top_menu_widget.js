...
function top_menu_widget_open_submenu(window) {
	var target = window || top_menu_widget
	if (!target) {
		return
	}

	if (target.new_game) {
		target.new_game.enabled = !game_features.gameui_hide_new_game_top_menu
	}

	if (target.display_options) {
		target.display_options.enabled = !screen.is_fullscreen_only
	}
}
